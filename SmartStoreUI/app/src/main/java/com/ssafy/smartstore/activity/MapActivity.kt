...
class MapActivity : AppCompatActivity(), OnMapReadyCallback,GoogleMap.OnMarkerClickListener {
    ...
    var locationCallback: LocationCallback = object : LocationCallback() {
        override fun onLocationResult(locationResult: LocationResult) {
            ...
            val loc = Location("")
            loc.latitude = 36.10830144233874
            loc.longitude = 128.41827450414362

            shopList.add(loc)

            if(shopList.size > 0) {
                val location = shopList[shopList.size-1]
...
//            if (mMap != null && checkLocationServicesStatus()) mMap!!.isMyLocationEnabled = true
    ...
    fun getCurrentAddress(latlng: LatLng): String {
        ...
        try {
            Log.d("latlng", "getCurrentAddress: ${latlng.latitude}  ${latlng.longitude}")
...
//        mMap!!.moveCamera(cameraUpdate)
    ...
    override fun onMarkerClick(p0: Marker): Boolean {
        ...
        val dialog = AlertDialog.Builder(this)
            .setTitle("원하시는 서비스를 골라주세요")
            .setPositiveButton("길찾기"){dialog,_ ->
                //구현
                findDirection()
            ...
            .setNegativeButton("전화걸기"){dialog,_ ->
    ...
    private fun findDirection() {
        val DEFAULT_LATITUDE = "36.10830144233874"
        val DEFAULT_LONGITUDE = "128.41827450414362"
